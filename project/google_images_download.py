...
class googleimagesdownload:
    ...
    def download(self,arguments):
        ...
        if __name__ != "__main__":
            ...
            if 'config_file' in arguments:
                ...
                total_errors = 0
                for rec in records:
                    paths,img_list,  errors = self.download_executor(rec)
                    for i in paths:
                        paths_agg[i] = paths[i]
                    if not arguments["silent_mode"]:
            ...
            # if the calling file contains params directly
                ...
                paths,img_list, errors = self.download_executor(arguments)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
                    if arguments['print_paths']:
        ...
        # for input coming from CLI
            ...
            if not arguments["silent_mode"]:
                if arguments['print_paths']:
                    print(paths.encode('raw_unicode_escape').decode('utf-8'))
        return paths_agg, img_list, errors
    ...
    def download_executor(self,arguments):
        ...
        return paths, img_list,total_errors
...
def main():
    ...
    for arguments in records:
        ...
        if arguments['single_image']:  # Download Single Image using a URL
            ...
            response = googleimagesdownload()
            paths,img_list,errors = response.download(arguments)  #wrapping response in a variable just for consistency
            total_errors = total_errors + errors
