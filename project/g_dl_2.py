...
class googleimagesdownload:
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        while count < limit+1:
            if len(image_objects) == 0:
                ...
                # if any of the links or images are none or corrupted 
                if (not object):
                    count += 1
                    i +=1
                    print("--------------------sick # image_objects=============",count)
    ...
    def download(self,arguments):
        ...
        if __name__ != "__main__":
            ...
            if 'config_file' in arguments:
                ...
                total_errors = 0
                for rec in records:
                    paths, errors, img_list = self.download_executor(rec)
                    for i in paths:
                        paths_agg[i] = paths[i]
                    if not arguments["silent_mode"]:
            ...
            # if the calling file contains params directly
                ...
                paths, errors, img_list = self.download_executor(arguments)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
                    if arguments['print_paths']:
                        print(paths.encode('raw_unicode_escape').decode('utf-8'))
                return paths_agg, errors, img_list
    ...
    def download_executor(self,arguments):
        ...
        return paths, total_errors, img_list
